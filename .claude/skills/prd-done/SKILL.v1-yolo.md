...
- [ ] **Review all automated feedback**: Check PR comments section for automated code review feedback (bots, linters, analyzers)
  - **Use multiple methods to capture all feedback**:
    ...
    - **Empty results are ambiguous without the third channel**: with all three fetched, an empty `/issues/{n}/comments` distinguishes "CodeRabbit reviewed and found nothing" from "CodeRabbit was rate-limited and never ran"
