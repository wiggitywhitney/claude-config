...
**Scope limit on the `@`-import half, stated here because this summary is what downstream work reads.** The durability result was measured for imports from `~/.claude/CLAUDE.md`: those survive `/compact`. Imports from a *project* `.claude/CLAUDE.md` were measured separately (2026-09-19): the project root returns with `load_reason: compact` but its own `@`-import — added mid-session, not present at `session_start` — produces no `include` record. That result narrows the field to two explanations rather than fully settling one; see the Resolved Questions section below for both measurements and the residual gap. An unqualified "`@`-imports are durable" read from this summary would wrongly extend the user-level result to the project-level case regardless.
...
| `include` | `@path` imports expanded from **any** CLAUDE.md, user-level or project-level | Startup, alongside the referencing file | Yes for user-level imports 🟢 — measured. **No `include` record for a project-level import added mid-session** 🟡 — measured, but doesn't rule out session-start-only loading, see Resolved Questions |
...
**Caveat on the `include` row, added 2026-08-04, narrowed 2026-09-19.** The 2026-08-03 measurement covered imports from `~/.claude/CLAUDE.md` only; the project `CLAUDE.md` returned with `load_reason: compact` but produced no `include` records for its own imports, leaving open whether project-level imports re-resolve, load only at `session_start`, or fail to resolve at all. A follow-up probe on 2026-09-19 narrowed this to two remaining readings without fully closing it — see finding 6 in [claude-config-load-findings.md](claude-config-load-findings.md) and the Resolved Questions section below.
...
### Whether project-level `@`-imports are re-injected after compaction — narrowed to two explanations, not fully settled

**Answer: a project-level import added mid-session produced no `include` record at `/compact`, but the probe cannot distinguish why.** Measured 2026-09-19 on this same claude-config session: a temporary `@.claude/probe-import.md` import was added to this repo's project `.claude/CLAUDE.md`, the same passive `InstructionsLoaded` hook was registered via `.claude/settings.local.json` (added mid-session, no restart — confirms the method note below), and `/compact` was run manually.
...
**The gap: the import was added after this session's `session_start`, so there is no record of it ever resolving into context before `/compact` ran.** The hook's log has no earlier entry for it — the only entry is the compaction one. So this result is consistent with either of two of finding 6's three original explanations: project-level imports never re-resolve after compaction, or they only resolve at `session_start` and this one was added too late to load in the first place. Both predict the identical zero-record outcome. The third explanation — the import doesn't resolve at all, ever — is not fully ruled out either, though it's the least likely reading given the file's own `compact` record proves the hook was watching it correctly.

**What this does settle, regardless of mechanism:** a project-level `@`-import cannot be assumed to behave like a user-level one for byte-budget purposes. **Consequence for Milestone C1's byte budget:** the 10,937 bytes previously flagged as depending on this answer must be tracked as first-load-only content if a project `CLAUDE.md` ever carries a real `@`-import — not as always-resident, always-loaded bytes the way the eleven user-level rules are. (This repo's project `.claude/CLAUDE.md` currently has zero `@`-imports; the historical ones that motivated finding 6 were already removed as a remedy before this probe ran.)

**To close the remaining gap:** run the same probe with the `@`-import present in `.claude/CLAUDE.md` from a fresh session's `session_start`, confirm it produces its own `include` record before doing anything else, then compact and check again. If it loads at startup but drops at compaction, that settles "never re-resolves." If it never loads at all, that points at the third explanation instead.
...
3. **Measure before deciding.** Two facts the policy depended on were unverified. The first — whether `@`-imports survive compaction — is now measured on the user-level half: imports from `~/.claude/CLAUDE.md` do survive. The project-level half is narrowed but not fully settled: a project-level import added mid-session produced no `include` record at compaction, which is consistent with either "never re-resolves" or "only resolves at `session_start`, added too late" (see Resolved Questions). Either way, Milestone C1's byte budget should record project-level `@`-import content as first-load-only, not always-loaded — the 10,937 bytes previously flagged as unmeasured have a measured lower bound now, even though the exact mechanism awaits a cleaner probe. The second, whether any `SKILL.md` exceeds the 5,000-token truncation cap, remains an *estimate* from `scripts/measure-context-load.sh` rather than a confirmed reading; the byte-to-token ratio is calibrated against one `/context` sample, so files near the cap could fall either side of it.
