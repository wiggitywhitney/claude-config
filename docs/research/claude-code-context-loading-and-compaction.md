...
**Caveat on the `include` row, added 2026-08-04, closed 2026-09-21.** The 2026-08-03 measurement covered imports from `~/.claude/CLAUDE.md` only; the project `CLAUDE.md` returned with `load_reason: compact` but produced no `include` records for its own imports. A 2026-09-19 follow-up narrowed the explanation space, and a 2026-09-21 fresh-session probe closed it: a project-level `@`-import present from `session_start` never enters context at all, by hook record or by direct inspection of what the session's opening context actually contained — see finding 6 in [claude-config-load-findings.md](claude-config-load-findings.md) and the Resolved Questions section below.
...
Setup: `@.claude/probe-import.md` was added to this repo's project `.claude/CLAUDE.md` and the passive `InstructionsLoaded` hook from the mid-session probe was left in `.claude/settings.local.json`, both in place before the session started — unlike the 2026-09-19 run, where the import was added after `session_start`.

The session-start payload carried 14 records: eleven `include` records for the user-level `~/.claude/CLAUDE.md`'s own `@`-imports (all parented to that file), one `session_start` record for the user-level `CLAUDE.md` itself, one `session_start` record for the project `.claude/CLAUDE.md`, and one unrelated `path_glob_match` record. **Zero records of any kind named `probe-import.md`** — no `include`, no `session_start`, nothing — despite the project `.claude/CLAUDE.md` itself loading correctly and the hook demonstrably firing for every other file in the same payload.

**Independent confirmation, outside the hook entirely:** this session's opening system-reminder block prints the full contents of every file that actually loaded into context — the project `.claude/CLAUDE.md` and all eleven of the user-level file's `@`-imports each appear as their own "Contents of ... :" block. `probe-import.md` does not appear anywhere in that block, even though its `@`-import line sat at the top of the project `.claude/CLAUDE.md` from the very first token of the session. The content was never injected — this isn't a hook gap, the file simply never entered context.

**This settles finding 6 completely.** Of the three original explanations, the fresh-session result rules out "only resolves at `session_start`, added too late to load" — this import *was* present at `session_start` and still didn't load. It also rules out "never re-resolves after compaction" as the operative mechanism, since there's no post-compaction question to ask about content that never loaded pre-compaction. What remains is the third explanation: **project-level `@`-imports from `.claude/CLAUDE.md` do not resolve at all**, at least not in this version. User-level `@`-imports from `~/.claude/CLAUDE.md` are the only ones this platform expands.

**Consequence for Milestone C1's byte budget, now unconditional rather than a hedge:** project-level `@`-import content never becomes always-loaded, always-resident context — it doesn't become resident at all. If a project `.claude/CLAUDE.md` ever carries a real `@`-import, its bytes should not be counted anywhere in the always-loaded budget; they're dead weight in the file, not a cost the session pays. (This repo's project `.claude/CLAUDE.md` currently has zero real `@`-imports outside this now-concluded probe.)
...
3. **Measure before deciding.** Two facts the policy depended on were unverified. The first — whether `@`-imports survive compaction — is now fully measured on both halves: imports from `~/.claude/CLAUDE.md` do survive; imports from a project `.claude/CLAUDE.md` never enter context in the first place, at `session_start` or otherwise, so compaction survival is moot for them (see Resolved Questions). Milestone C1's byte budget should exclude project-level `@`-import content entirely — it costs nothing, ever, not even at first load. The second, whether any `SKILL.md` exceeds the 5,000-token truncation cap, remains an *estimate* from `scripts/measure-context-load.sh` rather than a confirmed reading; the byte-to-token ratio is calibrated against one `/context` sample, so files near the cap could fall either side of it.
