...
**Produced for:** PRD #109 M2, consumed by M4 (Viktor's "never global" position) and M5 (skill consolidation and migration)
...
- **M5's migration risk is smaller than recorded, but not zero.** Deleting the YOLO files changes no behavior in those nine repos. It does leave nine sets of dangling symlinks, which need cleanup — a tidiness problem rather than a breakage problem.
...
**Interpretation:** Reduces the friction cost of the M5 consolidation — edits to skills are testable in-session. The exception matters when the migration script creates a directory that did not previously exist.
...
**Interpretation:** Any `.claude/commands/` files anywhere in the repo set are now a redundant third way to express a skill, and one more place a definition can hide. M7's cross-repo sweep should enumerate `.claude/commands/` alongside `.claude/skills/`.
...
4. **Add `.claude/commands/` to M7's cross-repo enumeration.** Commands and skills are one mechanism now, so a stray command file is an undiscovered skill definition.
