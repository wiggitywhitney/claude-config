...
- **Superseded 2026-08-02 by the full survey above, which found four accidental divergences rather than the two known when this was written.** The original note read: two pairs have confirmed behavioral divergence — `prd-done` and `prd-next` — and the remaining six were not examined beyond line counts, which showed differences of 3–23 lines each; do not read that as "no divergence." That caution was well founded. `prd-done` differs by only five lines and still hid four behavioral differences and two live bugs, and diffing the other six in full turned up two more accidental divergences plus a missing trailing newline. The line-count heuristic would have missed all three. `prd-next` YOLO is 184 lines against 302 interactive, replacing context-detection ceremony with an explicit **Autonomous Decision Protocol** — proceed-when and stop-when criteria. That protocol is the mechanism that makes reduced oversight safe, and it currently exists in exactly one of sixteen files. Generalizing it is higher-value than any individual bug fix.
