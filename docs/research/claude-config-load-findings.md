...
**Whether these two files actually double-load — settled 2026-09-19, see below.** The original compaction probe recorded the project `CLAUDE.md` returning with `load_reason: compact` but produced **no** `include` record for either import, while all twelve of `global/CLAUDE.md`'s imports did return as `include`. Three readings were open at the time and that evidence did not choose between them: project-level imports are not re-resolved after compaction the way user-level ones are; they load at `session_start` and the probe was installed too late to see it; or the import is not resolving at all. A follow-up probe (Milestone B1, PRD #109) settled it: **project-level `@`-imports are not re-resolved after compaction.** They load at `session_start`/`include` time but do not survive `/compact` the way user-level imports do — the first reading was correct. See the Resolved Questions section of [claude-code-context-loading-and-compaction.md](claude-code-context-loading-and-compaction.md) for the measurement.
...
**Resolved 2026-09-19, unaffected by this fix:** whether project-level `@`-imports are re-resolved after compaction at all. Milestone B1's follow-up probe (PRD #109) measured **no** — project-level `@`-imports do not survive `/compact`, only user-level ones do. That question outlived the two files that raised it, since any future project-level import inherits the answer: it must be counted as first-load-only, not always-loaded, for byte-budget purposes.
