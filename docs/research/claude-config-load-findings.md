...
**Whether these two files actually double-load — closed 2026-09-21: they don't, because the project-level `@`-import never resolves at all.** The original compaction probe recorded the project `CLAUDE.md` returning with `load_reason: compact` but produced **no** `include` record for either import, while all twelve of `global/CLAUDE.md`'s imports did return as `include`. A 2026-09-19 follow-up (Milestone B1, PRD #109) added a fresh `@`-import mid-session and got the same zero-record result, but couldn't rule out a session-start-only load timing it had missed. A 2026-09-21 fresh-session probe closed that gap: an `@`-import present in a project `.claude/CLAUDE.md` from `session_start` still produced no `include` record and, checked directly against what the session's opening context actually contained, never entered context at all. **Project-level `@`-imports don't resolve, full stop — this was never a double-load risk.** See the Resolved Questions section of [claude-code-context-loading-and-compaction.md](claude-code-context-loading-and-compaction.md) for the measurement.

**Byte exposure: zero.** These two files' `@`-references from the project `CLAUDE.md` never load, so they cost nothing beyond what `path_glob_match` already charges when their globs fire. The 10,937-byte figure once flagged as a possible 17% undercount in the inventory total does not apply. (The recorded total has since been corrected to 70,228 for a separate reason — see finding 7 — and that correction is unaffected by this closure.)
...
**Closed 2026-09-21, unaffected by this fix:** whether project-level `@`-imports resolve at all. Milestone B1's fresh-session probe (PRD #109) settled it — they don't, at `session_start` or after compaction. That question outlived the two files that raised it, and any future project-level import inherits the same answer: it costs zero always-loaded bytes, because it never becomes resident.
