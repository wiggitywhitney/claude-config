...
`~/Documents/Journal/CURRENT-CONTEXT.md` **was** `@`-referenced from `global/CLAUDE.md` and loaded every session (~305 tokens). It sat outside this repository, so #108's count of "11 `@`-referenced rule files" excluded it by construction and no sweep of `rules/` would ever have found it. **The reference was removed on 2026-08-05, per `PROGRESS.md`; its absence was re-confirmed on 2026-08-24** — `global/CLAUDE.md` now `@`-references only `~/.claude/rules/*`, which resolves back into this repository. The later date is when this document was corrected, not when the change happened. The method point survives the instance: a repository sweep still cannot prove what loads from outside it.
...
**Whether these two files actually double-load — narrowed 2026-09-19, not fully settled.** The original compaction probe recorded the project `CLAUDE.md` returning with `load_reason: compact` but produced **no** `include` record for either import, while all twelve of `global/CLAUDE.md`'s imports did return as `include`. Three readings were open at the time and that evidence did not choose between them: project-level imports are not re-resolved after compaction the way user-level ones are; they load at `session_start` and the probe was installed too late to see it; or the import is not resolving at all. A follow-up probe (Milestone B1, PRD #109) added a fresh `@`-import to a project `CLAUDE.md` mid-session and got the same zero-record result at `/compact` — but because that import was added after `session_start` rather than present from it, the probe cannot distinguish the first two readings; both predict the same outcome. **Either way, project-level `@`-imports cannot be assumed to behave like user-level ones for byte-budget purposes.** See the Resolved Questions section of [claude-code-context-loading-and-compaction.md](claude-code-context-loading-and-compaction.md) for the measurement and what a conclusive follow-up would need.
...
**Narrowed 2026-09-19, unaffected by this fix:** whether project-level `@`-imports are re-resolved after compaction at all. Milestone B1's follow-up probe (PRD #109) measured a zero-record result but could not pin down why — the import was added mid-session rather than present at `session_start`, so "never re-resolves" and "only resolves at `session_start`, added too late" remain equally consistent with the data. That question outlived the two files that raised it, since any future project-level import inherits the same open mechanism question: it must be counted as first-load-only, not always-loaded, for byte-budget purposes regardless of which explanation eventually wins.
