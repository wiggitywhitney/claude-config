...
- **Loop primitive** — YOLO `/prd-next` introduces a `/clear` → auto-resume loop that the careful variant doesn't have. After each milestone's work commits, `/clear` resets context and a SessionStart hook (`prd-loop-continue.sh` (removed 2026-08-18)) re-injects orientation so the fresh instance re-invokes `/prd-next`.
