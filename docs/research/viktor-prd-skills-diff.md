...
| Section | Status | What it does | Why it is there | Keep-or-collapse note for C1 |
|---|---|---|---|---|
| `### 2.5. Pre-PR Verification` | New | Launches a verification agent that maps every milestone's success criteria to observable code evidence using an Exists → Substantive → Wired check and reports gaps in a table before PR creation | `beb6866`, closing issue #72, to catch milestones marked complete without evidence | Long, and **not an extraction candidate despite appearing to be one.** The three-level check also lives in `~/.claude/rules/testing-rules.md`, so it reads as duplication — but this copy sits inside a prompt handed to a separate verification agent, and always-loaded rules are not a substitute for text that agent must receive directly. Replacing it with a pointer would leave the agent without its own definition. Established while cutting duplication from this file on 2026-09-04 |
