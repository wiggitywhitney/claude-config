...
7. ~~**Added by the Milestone B2 community-practices sweep (Decision 40).** Does a hook event exist that fires on **every user prompt**, not just session start or compaction — the mechanism a cited community case study credits with raising rule-compliance from ~20% to 84%? The shipped hook-event list enumerated on 2026-08-04 (Decision-log finding, `docs/research/claude-config-audit-decisions.md`) does not include anything named `UserPromptSubmit`, only `UserPromptExpansion` — check whether that event, some other name, or nothing at all fills this role before concluding the practice isn't adoptable.~~ **Resolved 2026-09-21 (Milestone B1).** `UserPromptSubmit` exists as its own hook event at 2.1.278, distinct from `UserPromptExpansion` — confirmed by `strings` on the shipped binary plus the fully wired `prompt.submit` code path, which can block a prompt or attach context to it via this event. **Scope of the "every prompt" claim:** confirmed for prompt submissions going through Claude Code's own `prompt.submit` dispatch (the CLI/interactive path this probe exercised); not verified for other channels (IDE/Desktop integrations, MCP-originated prompts, or anything mid-turn) or for whether any of those bypass this event. Not present in the 2026-08-04 enumeration at 2.1.221; confirmed present at 2.1.278, exact introduction version unknown. See [claude-config-audit-decisions.md](claude-config-audit-decisions.md) for the full re-check, including a retraction of an earlier draft's claim that the hook can rewrite prompt text — that behavior traces to the broader `prompt.submit` dispatch, not demonstrably to this hook. This is a real capability match for the community pattern this question was chasing, and it's a candidate Milestone C1 should evaluate for structural rule-compliance enforcement rather than relying on advisory `CLAUDE.md` instructions.
