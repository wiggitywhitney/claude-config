...
- Technology-specific gotcha files carry `paths:` frontmatter and load on their own when you touch a matching file. The full index of what exists and when each one fires lives in `~/.claude/rules/README.md` — read it when you want to know whether a rule already covers a technology.
