...
- Repos can opt into acceptance gate tests by adding `"acceptance_test"` to `.claude/verify.json` commands object. These run after standard PR verification passes, use `vals exec` for secret injection, and are advisory (never block PR creation) but require human review of results.
