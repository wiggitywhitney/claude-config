#!/usr/bin/env bash
# ABOUTME: Shared lib for git hooks — runs advisory CodeRabbit CLI review on branch changes
# ABOUTME: Always exits 0; caller decides whether to surface findings to the user
...
if [[ -n "$TIMEOUT_BIN" ]]; then
  REVIEW_OUTPUT=$("$TIMEOUT_BIN" 420 "$CODERABBIT_BIN" review --committed --base "$BASE_BRANCH" --dir "$PROJECT_DIR" 2>&1) || {
    EXIT_CODE=$?
  ...
  REVIEW_OUTPUT=$("$CODERABBIT_BIN" review --committed --base "$BASE_BRANCH" --dir "$PROJECT_DIR" 2>&1) || {
    EXIT_CODE=$?
    echo "CodeRabbit CLI review failed (exit $EXIT_CODE) — skipping"
    exit 0
  }
