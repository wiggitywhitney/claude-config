#!/usr/bin/env bash
# ABOUTME: Shared lib for git hooks — checks if all changed files are documentation-only
# ABOUTME: Reads file paths from stdin; exits 0 if all docs-only (safe to skip verification)
