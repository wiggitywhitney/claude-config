#!/usr/bin/env bash
# ABOUTME: Shared lib for git hooks — runs lint scoped to changed files only (staged or branch diff)
# ABOUTME: Supports JS/TS via ESLint and Go via golangci-lint; skips if no lintable files changed
