...
**Step 0:** The PRD skill is the battle-tested reference. Your job is identifying the minimal diff that transforms it into the issue equivalent — read `.claude/skills/prd-update-progress/SKILL.md` top-to-bottom to understand what it does, copy it verbatim to `.claude/skills/issue-update-progress/SKILL.md`, then make only the targeted edits needed for the issue context. Do not write from scratch; do not rewrite sections that work as-is (Decision 8, Decision 11). Before removing any section, verify it has no issue equivalent — when in doubt, ask (Decision 9). Read Decision 2 and Decision 3 in the Decision Log below (why comments over body edits; checkpoint goes to the relevant issue only). Note: the checkpoint comment format defined in this milestone is the contract that M4 (`/issue-next`) reads. Document it clearly in the SKILL.md so M4's implementer has an unambiguous reference. Read both `~/.claude/CLAUDE.md` and `.claude/CLAUDE.md` for any issue-specific procedures to integrate into this skill (Decision 10).
...
3. **Synthesize progress** — Use conversation context first, then `git log --oneline -10` as fallback, to derive: what got done since the last checkpoint, what the next concrete step is, and any open questions to preserve. Propose the complete checkpoint comment body to the user for confirmation. If the user edits the proposal, apply their edits before posting. Do not post the comment until the user approves the final text.
...
**Step 0:** The PRD skill is the battle-tested reference. Your job is identifying the minimal diff that transforms it into the issue equivalent — read `.claude/skills/prd-next/SKILL.md` top-to-bottom to understand what it does, copy it verbatim to `.claude/skills/issue-next/SKILL.md`, then make only the targeted edits needed for the issue context. Do not write from scratch; do not rewrite sections that work as-is (Decision 8, Decision 11). Before removing any section, verify it has no issue equivalent — when in doubt, ask (Decision 9). Read the checkpoint comment format in M3's completed SKILL.md before starting. The format defined there is the parsing contract for this skill — the sentinel is the literal string `## Progress Checkpoint` at the start of a comment body; do not match partial strings or variations. This milestone gates on M3 being committed. Read both `~/.claude/CLAUDE.md` and `.claude/CLAUDE.md` for any issue-specific procedures to integrate into this skill (Decision 10).
...
**Step 0:** The PRD skill is the battle-tested reference. Your job is identifying the minimal diff that transforms it into the issue equivalent — read `.claude/skills/prd-update-decisions/SKILL.md` top-to-bottom to understand what it does, copy it verbatim to `.claude/skills/issue-update-decisions/SKILL.md`, then make only the targeted edits needed for the issue context. Do not write from scratch; do not rewrite sections that work as-is (Decision 8, Decision 11). Before removing any section, verify it has no issue equivalent — when in doubt, ask (Decision 9). Read the global CLAUDE.md "Decision cascade" section. Read Decision 6 in the Decision Log below (cascade scope is all open issues and PRDs, same as the PRD equivalent). Read both `~/.claude/CLAUDE.md` and `.claude/CLAUDE.md` for any issue-specific procedures to integrate into this skill (Decision 10).
...
**Step 0:** The PRD skill is the battle-tested reference. Your job is identifying the minimal diff that transforms it into the issue equivalent — read `.claude/skills/prd-done/SKILL.md` top-to-bottom to understand what it does, copy it verbatim to `.claude/skills/issue-done/SKILL.md`, then make only the targeted edits needed for the issue context. Do not write from scratch; do not rewrite sections that work as-is (Decision 8, Decision 11). Before removing any section, verify it has no issue equivalent — when in doubt, ask (Decision 9). Read `rules/git-workflow.md` for the full CodeRabbit + `/code-review` gate that must be honored before merge. Read `rules/issue-juggling.md` for expected end state. Read both `~/.claude/CLAUDE.md` and `.claude/CLAUDE.md` for any issue-specific procedures to integrate into this skill (Decision 10).
...
5. **Update ROADMAP.md** — If `docs/ROADMAP.md` exists in the repo and the closed issue(s) appear in it, remove those entries (ROADMAP is forward-looking; completed work belongs in PROGRESS.md only). Commit alongside PROGRESS.md if updated. Note: this file is optional and repo-specific — claude-config does not have one; skip this step if the file is absent.
...
| 10 | Before writing each skill, read both `~/.claude/CLAUDE.md` and the project-level `.claude/CLAUDE.md` for issue-specific procedures to integrate | Relying only on the PRD spec or the duplicated PRD skill | The CLAUDE.md files contain issue-specific procedures (e.g., "GitHub Issues" section, PROGRESS.md rules, git workflow steps) that a skill should embody. These may not be visible from the PRD skill being duplicated. A CLAUDE.md audit before writing ensures the skill inherits all relevant mandated behaviors, not just those that mirror the PRD equivalent. |
| 11 | The work of writing each issue skill is identifying the minimal diff from its PRD counterpart — not composing a new skill | Rewriting sections that look PRD-specific; writing "something similar" from scratch | The PRD skills are battle-tested. Every section that works the same way for issues should be preserved unchanged — exact wording, edge-case handling, and structure included. The implementation process is: read the PRD skill top-to-bottom, copy it verbatim, identify what must change for the issue context, make only those targeted edits. Rewriting working sections introduces regressions and drift from proven behavior. |
