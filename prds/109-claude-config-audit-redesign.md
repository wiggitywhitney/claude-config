...
- **The live instrument**, for real frequency. `PermissionRequest`, `PermissionDenied`, and `PreToolUse` hooks log to `~/.claude-a3-scratch/permission-events.jsonl` via the gitignored `.claude/settings.local.json`. Installed and verified live on 2026-08-04. **Re-verify before relying on it rather than trusting that sentence** — it records a state observed once, the hooks live in a gitignored file that no commit would restore, and a version upgrade could rename the events. Confirm the three hooks are still present and that the log has grown recently; if it is dead, reinstall it and say so, because the window is shorter than the dates suggest. `PreToolUse` is the denominator, so report a prompt **rate**, not raw counts. **The log writes after a delay** — an empty file read moments after a prompt is indistinguishable from a dead probe, so wait before concluding it failed.
...
**Success criteria:**
- `docs/research/claude-code-permission-modes.md` exists, holding the `/research` output with sources, each claim marked verified-against-this-setup or documentation-only
- One provisional change is proposed with its risk stated, and if Whitney adopts it, a before/after prompt rate is reported with both window lengths named
- `scripts/measure-prompt-rate.sh` is committed with bats coverage, and the reported rates come from running it rather than from reading the log by eye (Decision 25)
- `docs/research/claude-config-permission-audit.md` exists, containing the frequency-ranked command inventory and the trigger taxonomy, with each class labeled settings-fixable or behavior-only, and the class-individuation rule stated
- Where the taxonomy is incomplete, it says so — an unfinished binary extraction is not a reason to hold the milestone (Decision 46)
...
- The scratch directory is deleted and the three instrument hooks are removed from `.claude/settings.local.json`, or their retention is agreed with Whitney explicitly
...
- Settings and permission surfaces — sandboxing, permission modes, `--settings`, `claudeMdExcludes`, managed-settings interaction. **`permission_mode` is observable per session from the hook payload**, so the running value can be measured rather than assumed.
...
- **Ten in `tests/bootstrap.bats` and `tests/install-git-hooks.bats`** — symlink installation and dispatcher tests exiting 127, "command not found". Squarely this milestone's subject matter.
...
- **Within Phase A:** Milestone A3 is independent. **Milestone A4 depends on Milestone A2**, because its Step 0 reads that load inventory and its own scope is defined as what the inventory does not cover. Milestone A4's former dependency on Milestone B3 was removed when its design work moved to Milestone C1.
...
- Is there a sandbox or permission-mode setting that eliminates the `simple_expansion` prompt class outright? *(Decided: Milestone C1. Inputs: Milestone A3's taxonomy and its provisional change plus measured before/after, Milestone B1's re-checked settings findings, Milestone B2's unattended-running answer.)*
