...
- **The shipped binary**, for the *complete* set of trigger classes rather than the ten that happened to be noticed. Recoverable with `strings -a` from `~/.local/share/claude/versions/<version>`. Version-pin every finding and re-check after an upgrade — this is undocumented internals and will drift. Per Decision 46 this is **evidence for Milestone C1, not a blocker**: if it stalls, ship the taxonomy labelled incomplete rather than waiting.
- **The live instrument**, for real frequency. `PermissionRequest`, `PermissionDenied`, and `PreToolUse` hooks log to `~/.claude-a3-scratch/permission-events.jsonl` via the gitignored `.claude/settings.local.json`, installed and verified 2026-08-04. `PreToolUse` is the denominator, so report a prompt **rate**, not raw counts. **The log writes after a delay** — an empty file read moments after a prompt is indistinguishable from a dead probe, so wait before concluding it failed. Its data improves the longer it runs; say plainly in the output how long the window was.
...
**What:** Give Whitney a provisional change she can adopt this week, and build the evidence base for Milestone C1's permanent decision.

**Why:** Approval prompts are a daily irritant and a direct hit to the "joy to use" goal. **The question underneath them is how other people run Claude unattended for hours, and this milestone did not originally ask it** — it was scoped to trimming prompts inside the current setup, which is a smaller question with a different answer. The mode a session runs in plausibly matters more than any allowlist, and nothing in the plan was checking that.

**To implement — half one, the relief pass (Decision 45):**
- Read the **official Claude Code documentation** on permission modes, sandbox, and auto-accept. Documentation is the primary source here, and it is where every plan-changing finding on 2026-08-03 came from. Verify each claim against Whitney's actual setup or a live session rather than trusting the page — `~/.claude/settings.json` currently sets `skipDangerousModePermissionPrompt` with no sandbox or default-mode configuration, and the live instrument reports the session's `permission_mode` directly, so the running value is observable rather than assumed.
- **Propose one provisional change, with its risk stated plainly, and let Whitney decide whether to adopt it.** Provisional means reversible and measured, not decided: Milestone C1 still owns the permanent settings decision (Decision 34), and adopting something this week must not let it harden into the answer by default.
- **Measure it.** The instrument is running, so capture a before window and an after window and report the change in prompt rate. State the length of each window — a few hours of data is a thin sample and must be labeled as one.
...
- Write the behavioral guidance that reduces prompts regardless of settings — prefer Grep/Glob/Read over Bash for search and inspection, write literal commands without variable expansion, use absolute paths instead of `cd X && ...`, and put genuine logic in a script file invoked plainly. **Propose where it lives; do not decide.** Weigh the recorded failure below: a plain rules file is a weak proposal.

**Success criteria:**
- Official documentation on permission modes, sandbox, and auto-accept is captured with sources, each claim marked verified-against-this-setup or documentation-only
...
| 45 | **Milestone A3 splits into a narrow relief pass now and the durable research later.** The narrow pass reads the official documentation on permission modes, sandbox, and auto-accept, verified against Whitney's actual setup, and proposes a provisional change measured with the live instrument. The full capability research stays in Milestone B1, and the permissions-and-unattended-running question is added to Milestone B2's existing community sweep (Decision 40). | Whitney's actual question is how her contemporaries run Claude for hours unattended, which Milestone A3 never asked — it is scoped to trimming prompts in the current setup. Two jobs separated by weeks cannot share one research pass: done now it is stale when Milestone C1 decides, done later it leaves her interrupted throughout. Splitting gives relief this week without letting a provisional setting harden into a design decision, and Milestone B1 re-running the same pass is the currency check rather than waste. Community practice stays late deliberately — read before the platform is understood, it supplies workarounds for solved problems. |
