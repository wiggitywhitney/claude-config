...
- **Settle whether `@`-imported rules survive compaction. Answered 2026-08-03: yes, they are re-injected.** Recorded in the Resolved Questions section of [the compaction research](../docs/research/claude-code-context-loading-and-compaction.md) with the captured payloads, and in [claude-config-load-findings.md](../docs/research/claude-config-load-findings.md) with the method. Milestone C1's classification policy may rely on `@`-import durability. Two details the result turns on, kept because a future reader will otherwise draw the wrong conclusion from the same data:
  - **Imports reappear with `load_reason: include`, not `compact`.** Only the two root memory files carry reason `compact`; their imports carry `include` plus a `parent_file_path` pointing back at the root. Compaction re-reads the roots from disk and re-resolving them pulls the imports along. Filtering for `load_reason == "compact"` alone shows two files and makes imports look dropped — almost certainly the origin of issue #24460.
