...
- Clone Michael's repos, owner `peopleforrester`:
  ...
  - **Selection method: recency, decided by a script, not by reading descriptions.** Enumerate his public repos with last-updated dates (`gh repo list peopleforrester --json name,description,updatedAt`), clone every one active within the last ~90 days, and judge relevance only after reading. A repo that turns out not to matter costs one clone and is discarded; a wrong guess made from a one-line description during planning costs a missing finding nobody knows is missing. This selection is not a decision that needs Whitney in the loop.
  - As of 2026-08-02 that filter yields at minimum: `claude-dotfiles` (production-ready Claude Code configurations, skills, and templates — his live setup may live here rather than in `llm-coding-workflow`), `Brain_spec_skills_claude` (Claude Code skills for spec-driven development), `observe-claude-code`, `mcp_best_practices`, and `agentic-covenants`. Re-run the enumeration rather than trusting this list; he has 21 public repos and was actively pushing on the day it was written.
...
- Michael's repo enumeration was re-run at clone time, and **every** repo it returned as recently active is cloned — not only `llm-coding-workflow`. The enumeration output is recorded alongside the clone list so the selection can be checked rather than taken on trust
