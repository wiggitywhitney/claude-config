...
**Step 0:** Read the approval-friction finding in [the decision log](../docs/research/claude-config-audit-decisions.md). **Eleven trigger classes are documented there as of 2026-08-03, up from four when this milestone was first written.** Do not restart the diagnosis; extend it. Three properties of that evidence set should shape the method:
...
- **Seven of the eleven were found only because Whitney screenshotted the prompt**, and one fires on the *content of a commit message* rather than on any command shape. So the mining must read prompt reason strings, not infer classes from how commands look. This is the strongest argument in the audit for evidence over reasoning.
- **Read `.claude/settings.local.json` as a data source, not just the transcripts.** Its 222 accumulated allowlist entries are fossils of past prompts — `Bash(done)`, `Bash(do echo:*)`, and a run of `Bash(__NEW_LINE_<hash>__ ...)` entries each record a multi-line or looped command that prompted per fragment. It is cheaper to read than transcripts and it survives session rotation. It is also the disproof of the obvious remedy: 222 entries did not prevent the eleven classes, because approving a literal command string generalizes to nothing.
...
- Is there a sandbox or permission-mode setting that eliminates the `simple_expansion` prompt class outright? *(Decided: Milestone C1. Inputs: Milestone A3's taxonomy, Milestone B1's settings findings.)*
