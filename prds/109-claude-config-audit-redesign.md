...
- **The always-loaded byte budget.** *Inputs: Milestone A2's inventory and the `@`-import compaction answer, now measured — imports **are** re-injected, so `@`-referenced rules cost their bytes in every session including after every compaction, and there is no discount to hope for.* Replaces PRD #43's unmet 150-line target. Two facts constrain it: roughly 40% of the startup skill listing is built-in and cannot be trimmed from this repo, so budget only the controllable portion; and imports must be counted by observed load rather than by globbing `rules/`, because at least one always-loaded import lives outside this repository. Decide the enforcement mechanism at the same time as the number — Decision 37 applies.
  ...
  **The 63,619-byte figure is now safe to budget against — it was not on 2026-08-04, and the reason is worth knowing (finding 6).** Two rules totalling 10,937 bytes were `@`-referenced from the *project* `.claude/CLAUDE.md`, which neither the inventory script nor the frontmatter checker read. Both are fixed and now scan every `CLAUDE.md`. The stray imports were removed rather than the `paths:` frontmatter, because `global/CLAUDE.md` already described both files as "reference pointer, not auto-loaded" — the remedy was aligning with a decision already recorded, not making a new one. **Had the opposite remedy been chosen the figure would be 74,556**, so if this milestone revisits either file's classification, the budget moves with it.
