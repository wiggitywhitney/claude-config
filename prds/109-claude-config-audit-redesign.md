...
- Clone Michael's repos, owner `peopleforrester`:
  ...
  - **Beyond that, select by recency, not by asking.** List his public repos with their last-updated dates. Anything recently active gets cloned; relevance is judged after reading, not guessed from the description. A repo that turns out not to matter costs a clone and is discarded — that is cheaper than a wrong guess made during planning, and it is not a decision that needs Whitney in the loop. Known strong candidates as of 2026-08-02: `claude-dotfiles` (production-ready Claude Code configurations, skills, templates; updated 2026-07-29 — his live setup may live here rather than in `llm-coding-workflow`) and `Brain_spec_skills_claude` (Claude Code skills for spec-driven development). Also active: `observe-claude-code`, `mcp_best_practices`, `agentic-covenants`. He has 21 public repos; recency is the filter, not completeness.
