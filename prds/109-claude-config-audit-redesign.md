...
**Start from a known defect rather than a blank sheet: two of this repo's own scripts are blind in the same way (finding 6, 2026-08-04).** `scripts/check-rule-frontmatter.sh` and `scripts/measure-context-load.sh` both derive the `@`-referenced set from `global/CLAUDE.md` alone, so neither sees the two imports in `claude-config/.claude/CLAUDE.md`. The checker consequently reports "All rules have exactly one loading mechanism" while two rules have both. **Teaching both to scan every `CLAUDE.md` is this milestone's work** — it is the `derive` remedy and is correct independent of what Michael's repo turns out to contain. Expect the checker to start failing once it can see, and expect that failure to be correct; the per-file remedy that follows is Milestone C1's decision, not this milestone's.
...
- **The always-loaded byte budget.** *Inputs: Milestone A2's inventory and the `@`-import compaction answer, now measured — imports **are** re-injected, so `@`-referenced rules cost their bytes in every session including after every compaction, and there is no discount to hope for.* Replaces PRD #43's unmet 150-line target. Two facts constrain it: roughly 40% of the startup skill listing is built-in and cannot be trimmed from this repo, so budget only the controllable portion; and imports must be counted by observed load rather than by globbing `rules/`, because at least one always-loaded import lives outside this repository. Decide the enforcement mechanism at the same time as the number — Decision 37 applies.
  ...
  **The inventory's 63,619-byte figure is not yet safe to budget against (finding 6, 2026-08-04).** Two rules totalling 10,937 bytes are `@`-referenced from the *project* `.claude/CLAUDE.md`, which neither the inventory script nor the frontmatter checker reads — both derive the referenced set from `global/CLAUDE.md` alone. The inventory therefore reports them as on-demand. If they load, the real total is about 17% higher, and it errs in the unsafe direction for a budget. **Resolve this before setting the number**, and note that the per-file remedy is itself a decision this milestone owns: drop the `@`-reference and rely on the globs, or drop `paths:` and pay the bytes deliberately.
