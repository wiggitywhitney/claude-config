...
### Decision 3: Global OTel Packaging Rule — API as peerDependency, Never Direct
- **Date**: 2026-02-21
- **Decision**: Added a global CLAUDE.md rule governing OpenTelemetry dependency classification across all projects. The OTel API must be a `peerDependency` in Node.js (never a direct dependency). SDK, instrumentation, and auto-instrumentation packages must never be direct production dependencies in any distributable package.
- **Rationale**: Research into the opentelemetry-js-contrib [GUIDELINES.md](https://github.com/open-telemetry/opentelemetry-js-contrib/blob/main/GUIDELINES.md) confirmed that `@opentelemetry/api` as a direct dependency causes silent trace loss — npm can install multiple copies, and OTel's global singleton pattern silently falls back to no-ops for incompatible versions. Real-world failures documented in Sentry, Azure SDK, and AWS Lambda. The rule is language-agnostic at the principle level ("libraries depend on the contract, applications choose the implementation") with a Node.js-specific `peerDependency` callout.
- **Impact**: Applies to this PRD's Milestone 2 (settings.json configuration) and to the commit-story-v2 telemetry agent spec (separate repo). Any future project adding OTel instrumentation inherits this rule. The telemetry agent spec should also add a `dependencyStrategy` config field so the agent asks during init whether the project is a service or distributable.
