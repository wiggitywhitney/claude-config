...
>
> Do not delete that branch and do not re-apply its commits yet. **PRD #109 M7b resolves this PRD end to end** — it assigns each piece of work on the branch one of three verdicts (trash / the code lives / the learnings live and the code does not), merges anything that survives, deletes the branch, closes this issue, and archives this file. It runs after PRD #109's M4, M5, and M6 spikes, because whether this architecture is still right is what those determine. The branch was also the only copy of five journal files, four of which were rescued to main on 2026-08-03.
**Research**:
...
>
> One hard constraint for any `-p` architecture: in non-interactive mode **repeated classifier blocks abort the session**, since there is no user to prompt. Auto mode is not a guarantee of an uninterrupted headless run, and `autoMode.environment` would need configuring before relying on it. **Sharpened by PRD #109, Decision 99, 2026-09-21: `autoMode.environment` only reduces false-positive blocks before they happen — the block-count thresholds themselves (3 consecutive / 20 total) are fixed and not configurable by any setting.** Configuring it lowers the odds of hitting that ceiling; it does not raise the ceiling.
>
