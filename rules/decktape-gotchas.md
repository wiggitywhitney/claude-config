...
- **Know which browser each install actually uses before blaming fonts.** An npx/local install uses Puppeteer's bundled Chromium by default — it does **not** automatically use the macOS Chrome you have installed. The Docker image uses Linux Chromium. So "install locally to fix font rendering" is only half true: if you need macOS font rendering specifically, pass `--chrome-path` pointing at the installed Chrome executable. If you must use Docker, install or mount the required fonts in the container. One reported case disliked the Docker output and got expected results locally, which is consistent with either Chromium-versus-Chrome or a missing-fonts cause.
