...
- **No Homebrew install path exists** — not a renamed tap, no tap at all. `brew install devbox` and `jetpack-io/devbox/devbox`/`jetify-com/devbox/devbox` do not work. The only install method is: `curl -fsSL https://get.jetify.com/devbox | bash`. An open GitHub feature request asking for brew support (#76, opened 2022) is still unresolved.
