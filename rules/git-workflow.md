...
- **CodeRabbit CLI manual invocation:** When you need to run a CodeRabbit CLI review outside the pre-push hook (e.g., reviewing a PRD or doc-only branch), use:
  ...
  - **Prefer `--agent` when Claude will be reading the output.** Run from inside Claude Code, the CLI prints `Notice: Detected claude environment. Use "coderabbit review --agent" for structured agent-friendly output` and then emits the human format anyway. That format wraps every file reference in an OSC 8 terminal hyperlink, so a path arrives as `]8;;vscode://file//abs/path:229docs/research/foo.md:229-231]8;;` and has to be untangled before it can be acted on. Observed on v0.7.3, 2026-08-18. Add `--agent` to the invocation when the findings are for Claude rather than for a person reading a terminal; keep the plain form when a human is reading along.
