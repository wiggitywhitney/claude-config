...
- **In zsh, `"$var:rest"` can be eaten by a history modifier, so a `git` object path built that way silently mangles.** `git cat-file -p "$tree:rules/writing-voice.md"` printed `ea51865...ules/writing-voice.md` — zsh read `:r` as the "root" modifier and consumed it, leaving a path that resolves to nothing. It does not error usefully: the surrounding pipeline returns empty, which reads as a genuine "no matches" result rather than a quoting bug. Observed 2026-09-10 while comparing a merge result, where it briefly looked like a rule had vanished from a merged file. Use `${tree}:path` with explicit braces, or pass the blob SHA from `git ls-tree` instead of building a `rev:path` string. This bites only in an interactive zsh shell, not in a `#!/usr/bin/env bash` script — which is why it survives review of the script and appears when someone runs the same line by hand.
