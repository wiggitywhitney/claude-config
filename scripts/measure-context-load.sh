...
# Scans the configured global and project CLAUDE.md files — not every CLAUDE.md that
# could carry imports; see the scope limit noted at PROJECT_CLAUDE_MD above — and records
...
printf 'For rule files, the loading mechanism determines the byte cost, and it determines compaction survival\n'
printf 'in the two cases that were measured: `paths:`-scoped rules do **not** survive, and rules imported from\n'
printf '`global/CLAUDE.md` do. **Imports from `.claude/CLAUDE.md` are unmeasured** and the rows below report them\n'
printf 'as `untested` rather than inferring a verdict from the global result.\n'
...
while IFS= read -r file; do
  ...
  if [[ "$scoped" == yes && "$referenced" == yes ]]; then
    mech='**both — defect**'; loaded='yes'
    # The compaction verdict follows the import's source here for the same reason it does
    # in the plain include branch below. A both-mechanisms rule imported only from the
    # project CLAUDE.md would otherwise be reported as surviving, which is precisely the
    # unmeasured case — asserting a measured verdict for it in the row that flags a defect.
    if [[ "$ref_source" == global ]]; then
      survives='yes'
      why='Carries `paths:` **and** is `@`-referenced from `global/CLAUDE.md`, so it loads twice. Violates the one-mechanism rule.'
    else
      survives='untested'
      why='Carries `paths:` **and** is `@`-referenced from `.claude/CLAUDE.md`, so it loads twice. Violates the one-mechanism rule. **Project-level re-resolution after compaction is unmeasured.**'
    fi
...
printf '\n**Configuration and rules subtotal:** %s bytes.\n' \
  "$((claude_md_bytes + project_md_bytes + total_included_global + total_included_project + total_bare))"
printf 'Both `CLAUDE.md` files, their imports, and any bare rule files. This is the figure\n'
printf 'Decision 42 records. The project `CLAUDE.md` and its imports belong in it because they\n'
printf 'load every session; omitting them is how the total was undercounted before 2026-08-04.\n'
printf '\n**Always-loaded total including skill descriptions:** %s bytes.\n' \
  "$((claude_md_bytes + project_md_bytes + total_included_global + total_included_project + total_bare + skill_desc_total))"
printf 'Skill descriptions load into the startup listing every session, so a byte budget that\n'
printf 'omits them understates the real cost. They are shown as a separate total rather than\n'
printf 'folded into the subtotal above, because the subtotal is what earlier measurements and\n'
printf 'Decision 42 refer to and silently changing its meaning would break that comparison.\n'
printf 'Both figures are repository-only; see the bound stated below.\n'
