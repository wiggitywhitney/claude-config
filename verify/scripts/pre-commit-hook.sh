...
reason = f'Verification failed at phase: {phase}. Fix the underlying code to resolve the error. NEVER add lint-disable comments, @ts-ignore, type:ignore, eslint-disable, or similar suppression annotations to bypass the check. Fix the actual problem in the code.\n\n{output}'
result = {
    'hookSpecificOutput': {
